...
```ts
  ...
  27 | 
  28 | 
  29 | test('logout scenario @master @logout', async({page})=>
  30 | {
  ...
  37 | const isexits=await logoutpage.iscontinuebuttonexists()
  38 | console.log(isexits)
> 39 | expect(isexits).toBeTruthy()
     |                 ^ Error: expect(received).toBeTruthy()
  40 | 
  41 | 
  42 | })
